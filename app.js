...
const shapeInfoPanel = document.getElementById("shape-info-panel");
const shapeInfoGrid  = document.getElementById("shape-info-grid");
...
const polyEdgesEl   = document.getElementById("poly-edges");
...
function getHint(t) {
  return {
    ...
    polygon:   "Atur jumlah sisi, lalu klik titik-titik di canvas.",
    scanline:  "Atur jumlah sisi, lalu klik titik-titik di canvas.",
...
canvas.addEventListener("mousedown", e => {
  ...
  if (activeTool === "polygon" || activeTool === "scanline") {
    ...
    const targetEdges = Number(polyEdgesEl.value) || 3;
    setStatus(`Titik ${polygonPts.length}/${targetEdges} ditambahkan.`);
    if (polygonPts.length >= targetEdges) {
      finishPolygon();
    }
    return;
  }
...
if (finishPolyBtn) finishPolyBtn.addEventListener("click", finishPolygon);
...
// ── SHAPE INFO ────────────────────────────────────────────────
/**
 * Returns an array of info items { label, value, unit, color } for a shape.
 * All measurements are in pixels (canvas units).
 */
function getShapeInfo(shape) {
  ...
  // Helper: format number with up to 2 decimals
  const fmt  = v => Number.isInteger(v) ? String(v) : v.toFixed(2);
  const fmt0 = v => Math.round(v).toLocaleString();

  switch (shape.type) {
    case "point": {
      const wx = shape.cx + t.tx;
      const wy = shape.cy + t.ty;
      items.push({ label: "Pos X",    value: fmt(wx),      unit: "px",   color: "accent" });
      items.push({ label: "Pos Y",    value: fmt(wy),      unit: "px",   color: "accent" });
      items.push({ label: "Ukuran",   value: fmt(shape.pixelSize + 2), unit: "px", color: "" });
      items.push({ label: "Total Px", value: "1",          unit: "titik",color: "green" });
      break;
    }

    case "line":
    case "bresenham": {
      const p1w = applyMatrix(shape.p1.x, shape.p1.y, t, shape.cx, shape.cy);
      const p2w = applyMatrix(shape.p2.x, shape.p2.y, t, shape.cx, shape.cy);
      const dx = p2w.x - p1w.x, dy = p2w.y - p1w.y;
      ...
      const tebal  = shape.pixelSize;
      const totalPx = steps * tebal;

      items.push({ label: "Panjang",  value: fmt(panjang), unit: "px",   color: "accent" });
      items.push({ label: "Sudut",    value: fmt(sudut),   unit: "°",    color: "amber"  });
      items.push({ label: "P1",       value: `(${fmt(p1w.x)}, ${fmt(p1w.y)})`, unit: "", color: "" });
      items.push({ label: "P2",       value: `(${fmt(p2w.x)}, ${fmt(p2w.y)})`, unit: "", color: "" });
      items.push({ label: "∆x",       value: fmt(Math.abs(dx)), unit: "px", color: "" });
      items.push({ label: "∆y",       value: fmt(Math.abs(dy)), unit: "px", color: "" });
      items.push({ label: "Tebal",    value: String(tebal), unit: "px",  color: "" });
      items.push({ label: "Total Px", value: fmt0(totalPx), unit: "px",  color: "green", full: true, bar: true, barMax: 5000 });
      break;
    }
    ...
    case "circle": {
      ...
      const diameter  = 2 * r;
      // Midpoint circle total pixel count ≈ circumference in pixels = 2πr
      const totalPx   = Math.round(keliling);

      items.push({ label: "Radius",    value: fmt(r),        unit: "px",   color: "accent" });
      items.push({ label: "Diameter",  value: fmt(diameter), unit: "px",   color: "accent" });
      items.push({ label: "Keliling",  value: fmt(keliling), unit: "px",   color: "amber"  });
      items.push({ label: "Luas",      value: fmt(luas),     unit: "px²",  color: "green"  });
      items.push({ label: "Cx",        value: fmt(shape.cx + t.tx), unit: "px", color: "" });
      items.push({ label: "Cy",        value: fmt(shape.cy + t.ty), unit: "px", color: "" });
      items.push({ label: "Total Px",  value: fmt0(totalPx), unit: "px",   color: "green", full: true, bar: true, barMax: 3000 });
      break;
    }

    case "ellipse": {
      const rx = shape.rx * sc;
      const ry = shape.ry * sc;
      // Ramanujan's approximation for ellipse perimeter
      const h = ((rx - ry) / (rx + ry)) ** 2;
      const keliling = Math.PI * (rx + ry) * (1 + (3*h)/(10 + Math.sqrt(4 - 3*h)));
      const luas = Math.PI * rx * ry;
      const totalPx = Math.round(keliling);

      items.push({ label: "Radius X",  value: fmt(rx),       unit: "px",   color: "accent" });
      items.push({ label: "Radius Y",  value: fmt(ry),       unit: "px",   color: "accent" });
      items.push({ label: "Keliling",  value: fmt(keliling), unit: "px",   color: "amber"  });
      items.push({ label: "Luas",      value: fmt(luas),     unit: "px²",  color: "green"  });
      items.push({ label: "Cx",        value: fmt(shape.cx + t.tx), unit: "px", color: "" });
      items.push({ label: "Cy",        value: fmt(shape.cy + t.ty), unit: "px", color: "" });
      items.push({ label: "Eksen Maj", value: fmt(Math.max(rx,ry)*2), unit: "px", color: "" });
      items.push({ label: "Eksen Min", value: fmt(Math.min(rx,ry)*2), unit: "px", color: "" });
      items.push({ label: "Total Px",  value: fmt0(totalPx), unit: "px",   color: "green", full: true, bar: true, barMax: 4000 });
      break;
    }

    case "rect": {
      const lebar  = shape.hw * 2 * sc;
      const tinggi = shape.hh * 2 * sc;
      const keliling = 2 * (lebar + tinggi);
      const luas     = lebar * tinggi;
      // DDA per 4 sisi total pixel ≈ 2*(lebar+tinggi)
      const totalPx = Math.round(keliling);

      items.push({ label: "Lebar",     value: fmt(lebar),    unit: "px",   color: "accent" });
      items.push({ label: "Tinggi",    value: fmt(tinggi),   unit: "px",   color: "accent" });
      items.push({ label: "Keliling",  value: fmt(keliling), unit: "px",   color: "amber"  });
      items.push({ label: "Luas",      value: fmt(luas),     unit: "px²",  color: "green"  });
      items.push({ label: "Cx",        value: fmt(shape.cx + t.tx), unit: "px", color: "" });
      items.push({ label: "Cy",        value: fmt(shape.cy + t.ty), unit: "px", color: "" });
      items.push({ label: "Asp.Ratio", value: lebar > 0 && tinggi > 0 ? fmt(lebar/tinggi) : "—", unit: "", color: "" });
      items.push({ label: "Diagonal",  value: fmt(Math.hypot(lebar, tinggi)), unit: "px", color: "" });
      items.push({ label: "Total Px",  value: fmt0(totalPx), unit: "px",   color: "green", full: true, bar: true, barMax: 5000 });
      break;
    }

    case "polygon": {
      const worldPts = shape.pts.map(p => applyMatrix(p.x, p.y, t, shape.cx, shape.cy));
      ...
      const totalPx = Math.round(perimeter);

      items.push({ label: "Titik",     value: String(n),     unit: "buah", color: "accent" });
      items.push({ label: "Perimeter", value: fmt(perimeter),unit: "px",   color: "amber"  });
      items.push({ label: "Luas",      value: fmt(area),     unit: "px²",  color: "green"  });
      items.push({ label: "Cx",        value: fmt(shape.cx + t.tx), unit: "px", color: "" });
      items.push({ label: "Cy",        value: fmt(shape.cy + t.ty), unit: "px", color: "" });
      items.push({ label: "Total Px",  value: fmt0(totalPx), unit: "px",   color: "green", full: true, bar: true, barMax: 5000 });
      break;
...
function updateShapeInfoPanel(shape) {
  if (!shape) {
    shapeInfoPanel.style.display = "none";
    shapeInfoGrid.innerHTML = "";
    return;
  }
  shapeInfoPanel.style.display = "";
  const items = getShapeInfo(shape);

  const COLOR_MAP = { accent: "accent", green: "green", amber: "amber", red: "red", "": "" };

  // Common header: type chip + index
  const idx = shapes.indexOf(shape) + 1;
  const typeLabel = { point:"Titik", line:"Garis DDA", bresenham:"Garis Bresnh.", circle:"Lingkaran", ellipse:"Elips", rect:"Persegi", polygon:"Poligon" }[shape.type] || shape.type;

  let html = `<div class="si-section-title">📐 ${typeLabel} #${idx}</div>`;

  for (const item of items) {
    if (item.bar) {
      // Pixel bar row
      const pct = Math.min(100, (parseInt(item.value.replace(/,/g,''), 10) / item.barMax) * 100);
      html += `
        <div class="si-pixel-bar">
          <span class="si-pixel-bar-label">Total Piksel</span>
          <span class="si-pixel-bar-val">${item.value}</span>
          <div class="si-pixel-bar-track"><div class="si-pixel-bar-fill" style="width:${pct}%"></div></div>
          <span class="si-unit">${item.unit}</span>
        </div>`;
    } else {
      const fullClass = item.full ? " full" : "";
      const colorClass = item.color ? ` ${item.color}` : "";
      html += `
        <div class="si-card${colorClass}${fullClass}">
          <div class="si-label">${item.label}</div>
          <div class="si-value${colorClass ? ` ${item.color}` : ''}">${item.value}<span class="si-unit">${item.unit}</span></div>
        </div>`;
    }
  }

  shapeInfoGrid.innerHTML = html;
}
